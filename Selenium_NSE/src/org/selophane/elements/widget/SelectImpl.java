...
public class SelectImpl extends ElementImpl implements Select {
     ...
     */
    public SelectImpl(final WebElement element) {
        super(element);

		Log.trace("SelectImpl :: Constructor");
    ...
    public void selectByVisibleText(String text) {
        ...
        for (WebElement option : options) {
            state = state.recognizeNewState(setSelected(option));
            if (!isMultiple() && state == State.SELECTED) {
            	//commenting as already logged in function: setSelected
            	//Log.debug("SelectImpl.selectByVisibleText :: Selected option: " + option.getText());
                return;
            }
        }
    ...
    public void selectByIndex(int index) {
        ...
        for (WebElement option : getOptions()) {
            if (match.equals(option.getAttribute("index"))) {
                ...
                if (!isMultiple()  && state == State.SELECTED) {
                	//commenting as already logged in function: setSelected
                	//Log.info("SelectImpl.selectByIndex :: Value selected: "
                	//		+ option.getText() + ", at index: " + index);
    ...
    private State setSelected(WebElement option) {
        if (!option.isDisplayed()) {
        	Log.info("SelectImpl.setSelected :: option is not visible: " + option.getText());
        	
            return State.NOT_VISIBLE;
        }
        if (!option.isEnabled()) {
        	Log.info("SelectImpl.setSelected :: option is disabled: " + option.getText());
        ...
        Log.info("SelectImpl.setSelected :: option selected is: " + option.getText());
